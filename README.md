...
### Пример 2: Каскадное удаление зависимых разрешений
...
// Удаляем WRITE
selected = togglePermission(permissions[1], permissions, selected);
// Результат: selected = [READ]
// WRITE удален
// DELETE удален каскадно (т.к. зависит от WRITE)
// READ остался (его зависимости удалены)
```
...
### При удалении разрешения (каскадное удаление):

1. Удаляется само разрешение из `selectedPermissions`
2. **Каскадно** удаляются все разрешения, которые зависят от удаляемого
   - Рекурсивно ищутся все разрешения, имеющие удаляемое в своих зависимостях
   - Все найденные разрешения также удаляются
3. Для каждого удаленного разрешения проверяются его зависимости:
   - Используется ли зависимость оставшимися разрешениями в `selectedPermissions`
   - Если зависимость не используется — она удаляется
   - Если зависимость используется — она остается в списке

**Пример каскадного удаления:**
- Если выбраны: READ, WRITE (зависит от READ), DELETE (зависит от READ и WRITE)
- При удалении READ → каскадно удаляются WRITE и DELETE
- При удалении WRITE → каскадно удаляется DELETE, остается READ
